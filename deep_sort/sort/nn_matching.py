...
def _nn_cosine_distance(x, y):
    ...
    # 输出每个trackid中feature对应的dection的feature最小的余弦值
...
class NearestNeighborDistanceMetric(object):
    ...
    def partial_fit(self, features, targets, active_targets):
        ...
        for feature, target in zip(features, targets):
            # samples字典中，存在的tatget[track_id]就加入新feature
            # 否则建立新的target[track_id],并加入新的feature
            # 这里相当于不断加入新匹配的feature值，并保存旧的
            self.samples.setdefault(target, []).append(feature)
            # 对应目标下添加新的feature，更新feature集合
            # 目标id  :  feature list
            if self.budget is not None:
                # 取后budget个dection的feature
                self.samples[target] = self.samples[target][-self.budget:]
            # 设置预算，每个track最多多少个feature，超过直接忽略之前的

        # 筛选激活的目标
        # 只保存标记为confirmed的track
    ...
    def distance(self, features, targets):
        ...
        """
        # target(track_id[]), features < 100dim
        cost_matrix = np.zeros((len(targets), len(features)))
        for i, target in enumerate(targets):
            # 计算每个terack_id中所有的（包括预测的feature）与所有检测出目标的feature 
            #的余弦距离代价矩阵，每行代表一个track对应的最小一个feature的余弦值
