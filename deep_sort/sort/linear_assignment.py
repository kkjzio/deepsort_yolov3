...
def min_cost_matching(
    ...
    # -----------------------------------------
    # cost_matrix每行对应一个target中的track_id 其又对应的dection检测的所有feature的最小余弦值
    # 行对应track_id（不是顺序的，序列对应metric._metric中的track_id）
    # 列对应detection中的feature的id
    ...
    # 注意结果是1-iou，所以越小越好
    # -----------------------------------------
    # numpy里的用法，将cost_matrix里大于max_distance的值强制变为max_distance + 1e-5
    # 这里是论文的b2
    cost_matrix[cost_matrix > max_distance] = max_distance + 1e-5
    # 到这里为止实际上只用了论文中的d2，也就是lamda为0，只用了外观距离 
    # 但是外观距离任然受b1（马氏距离的开关）和b2（外观模型距离最大值）的控制
...
def gate_cost_matrix(
    ...
    for row, track_idx in enumerate(track_indices):
        ...
        # 相当于cost_matrix*gating_distance，使得大于其卡方分布表的值置于一个极大值
        # 这里是论文的b1开关
